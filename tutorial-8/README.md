...
In the previous tutorial I pointed out that we would only be tuning a blocking *Web Application Firewall*. People running *ModSecurity* in pure *monitoring mode*, will eliminate individual false alarms one after the other. But very ofen and despite really good intentions the administrator gives up in the end, with no clear objective in mind and overcome by the sheer quantity of false alarms.
...
* We employ authentication and only take into account requests passing this gate.
...
However, these considerations digress from the actual topic of tuning. If you are unsure, give it a spin with the sample log files prepared for you to practice.
...
This means that out of the big heap of data we have to deal with five requests and will then be able to lower the anomaly limit immediately afterwards without having to account for legitimate users being blocked. In practice, I recommend exercising extreme caution at this stage (so, in our case, not something like an immediate reduction to 90, but perhaps to 100; a safety margin is a good idea). And even if some things are being blocked now and then, we can be certain that it will be a rare event, since the vast majority of requests on the service being managed have significantly lower anomaly scores as we see in the stats.

Without these considerations, you have 5,400 rule alerts and you do not know where to start.

###Step 5: Determining the ModSec Core Rules violated and deriving ignore rules

Our immediate goal is the five requests with an anomaly score of 91. Which requests are they?
...
This script is able to read and interpret *ModSecurity* alerts. *-h* gives you an overview of the different ways it can be used. We are primarily interested in the script’s ability to independently generate the *ignore rules* we want, saving us a lot of effort. The variants of this rule we saw in the previous tutorial can be accessed in *path*, *parameter* and *combined* modes. There is no mode for the complicated scoring suppression rules however. Here’s the script in action:
...
For each rule, the script lists the total number of rule violations and then proposes an *ignore rule* which can be included in the *Apache configuration*; only the *rule ID* for the *ignore rule* has to be modified usually. Next to one another, the rules are a bit hard to read, which is why I in practice I often summarize it by hand as you can see below (the *sort* command comes in handy as well). What’s important is to add comments to the rules, since the individual numbers don’t tell us much at all:
...
This is now a more compact and legible block of *ignore rules* associated with the same parameter on the same path. But our five requests with an anomaly score of 91 indicate another parameter being violated that we should not forget: REQUEST_COOKIES:X0_org. This alert shows the elegance of the approach being proposed. In addition to the five rule violations caused by this cookie in our score 91 example, in the entire log file it is responsible for over 2,000 alarms, almost half of all rule violations.
